...
async def router_node(state: SpecialReportState, config: RunnableConfig) -> SpecialReportState:
    ...
    if directive == RouterDirective.STAY:
        ...
        logger.info(
            f"DEBUG: Preparing to find next section. Current section states: {state.get('section_states', {})}"
        )
        ...
        if current_section_id in state.get("section_states", {}):
            ...
            logger.info(
                f"TRANSITION_DEBUG: Section {current_section_id} final state - status: {current_state.status}, has_content: {bool(current_state.content)}"
            )

        next_section = get_next_unfinished_section(state.get("section_states", {}))
        logger.info(
            f"DEBUG: get_next_unfinished_section decided the next section is: {next_section}"
        )
        ...
        if next_section:
            ...
            if previous_section != next_section:
                ...
                logger.info(
                    f"Preserved short_memory on same-section NEXT directive for {next_section.value}"
                )

            # Get context for new section
            logger.debug(
                f"DATABASE_DEBUG: Router calling get_context for section {next_section.value}"
            )
        ...
        try:
            ...
            # Get context for new section
            logger.debug(
                f"DATABASE_DEBUG: Router calling get_context for modify section {new_section.value}"
            )
            context = await get_context.ainvoke(
                {
                    "user_id": state["user_id"],
            ...
            logger.debug(
                f"DATABASE_DEBUG: get_context returned for modify section {new_section.value}"
            )
