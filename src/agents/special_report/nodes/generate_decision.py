...
The AI's reply was: {current_reply}"""
    ...
    try:
        ...
        decision_config = RunnableConfig(
            configurable={"stream": False}, tags=["decision_analysis", "internal"]
        )
        ...
        logger.info(f"🎯 Router directive: {decision_output.router_directive}")
        logger.info(f"📝 User satisfaction feedback: {decision_output.user_satisfaction_feedback}")
        logger.info(f"😊 Is satisfied: {decision_output.is_satisfied}")
        logger.info(f"💾 Section update provided: {bool(decision_output.section_update)}")
        ...
        if decision_output.is_requesting_rating:
            ...
            if not decision_output.section_update:
                logger.warning(
                    "AI requested rating but provided no section_update - may need content"
                )

            state["is_awaiting_rating"] = True
            logger.info(
                f"State updated: is_awaiting_rating set to {decision_output.is_requesting_rating}"
            )
        else:
        ...
        logger.info(
            f"DEBUG_DECISION_NODE: Decision generated successfully: {decision_output.router_directive}"
        )
