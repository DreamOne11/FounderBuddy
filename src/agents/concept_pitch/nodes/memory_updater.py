"""Memory updater node for Concept Pitch Agent."""
...
async def memory_updater_node(state: ConceptPitchState, config: RunnableConfig) -> ConceptPitchState:
    """
    ...
    if agent_out and agent_out.should_save_content:
        ...
        section_to_model_map = {
            SectionID.SUMMARY_CONFIRMATION: None,  # No specific model for summary
            SectionID.PITCH_GENERATION: None,      # No specific model for generation
            SectionID.PITCH_SELECTION: None,       # No specific model for selection
            SectionID.REFINEMENT: None,            # No specific model for refinement
            SectionID.IMPLEMENTATION: None,        # No specific model for implementation
        }
        ...
        state.setdefault("section_states", {})[section_id] = SectionState(
