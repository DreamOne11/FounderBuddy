...
async def get_system_prompt(user_id: str | None = None) -> str:
    ...
    Returns:
        System prompt string with user context if available
    """
    print(f"[DEBUG] get_system_prompt called with user_id: {user_id}")
    ...
    if user_id:
        print(f"[DEBUG] Fetching user context for user_id: {user_id}")
        try:
            user_context = await get_user_context(user_id)
            print(f"[DEBUG] User context fetched successfully, length: {len(user_context)}")
            return user_context + "\n" + base_instructions
        except Exception as e:
            print(f"[DEBUG] Error fetching user context: {e}")
            # If user context fails, fall back to default
            return base_instructions
    else:
        print("[DEBUG] No user_id provided, using default system prompt")
...
async def wrap_model(model: BaseChatModel, user_id: str | None = None) -> RunnableSerializable[AgentState, AIMessage]:
    ...
    print(f"=== RESEARCH ASSISTANT SYSTEM PROMPT ===")
...
async def acall_model(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    print(f"[DEBUG] acall_model - state user_id: {state.get('user_id')}")
    print(f"[DEBUG] acall_model - config user_id: {config['configurable'].get('user_id')}")
    print(f"[DEBUG] acall_model - final user_id: {user_id}")
