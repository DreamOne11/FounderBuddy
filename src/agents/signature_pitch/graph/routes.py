"""Routing logic for Signature Pitch Agent."""
...
def route_decision(
...
) -> Literal["generate_reply", "implementation"] | None:
    """
    ...
    if state.get("finished", False):
        logger.info("Graph control - Signature Pitch completed, moving to implementation")
    ...
    logger.info(
        f"Graph control - Router directive: {directive}, Has pending input: {has_pending_user_input()}"
    )
    ...
    # 3. NEXT/MODIFY directive - section transition
        ...
        if has_pending_user_input():
            logger.info(
                "Graph control - Pending user input with NEXT/MODIFY directive, going to generate_reply"
            )
            return "generate_reply"
        ...
        logger.info(
            "Graph control - NEXT/MODIFY directive, going to generate_reply for new section"
        )
        return "generate_reply"
