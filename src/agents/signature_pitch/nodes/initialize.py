...
async def initialize_node(
    ...
    if "user_id" not in state or not state["user_id"]:
        ...
        if "user_id" in configurable and configurable["user_id"]:
            ...
            state["user_id"] = 1
            logger.warning(
                f"Initialize node - No user_id in config, using default: {state['user_id']}"
            )
    ...
    if "thread_id" not in state or not state["thread_id"]:
        ...
        if "thread_id" in configurable and configurable["thread_id"]:
            ...
            logger.warning(
                f"Initialize node - No thread_id in config, generated: {state['thread_id']}"
            )
    ...
    logger.info(
        f"Initialize node - Final user_id: {state['user_id']}, thread_id: {state['thread_id']}"
    )
    return state
