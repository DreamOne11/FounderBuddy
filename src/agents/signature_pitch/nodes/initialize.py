...
async def initialize_node(state: SignaturePitchState, config: RunnableConfig) -> SignaturePitchState:
    ...
    if "current_section" not in state:
        state["current_section"] = SignaturePitchSectionID.CLARITY
    ...
    if "canvas_data" not in state:
        # Initialize with empty dict for CAPSTONE framework data
        state["canvas_data"] = {}
        
    if "legacy_canvas_data" not in state:
        state["legacy_canvas_data"] = SignaturePitchData()
